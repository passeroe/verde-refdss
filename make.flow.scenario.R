...
inputs_hp <- list(
  spring <- list("spring","No",NA,-20,0),
  summer <- list("summer","No",NA,-10,0),
  fall <- list("fall","No",NA,10,0),
  winter <- list("winter","No",NA,0,0)
)
...
# Save scenario info for internal use
if(SaveScene == "Yes"){
  ...
  list.save(scene_info,file=paste(reach_wd,"flow_scenarios","/",scene_name,"_q",".rdata",sep="")) # output
}
