...
#veg_list <- c("tall_trees","popufre") # vegetation group list
...
# Plotting Labels. Labels need to match the order of veg_list and scene_names.
scene_names_full <- c("Baseline","Scenario A","Scenario B")
...
scene_labs <- c("scenario_A"="Scenario A","scenario_B"="Scenario B")
...
chg_f_plt <- ggplot(data = fish_pc_tab, aes(x = factor(month), y = per_chg, 
  ...
  #facet_wrap(vars(scene),dir = "v",labeller = labeller(scene =scene_list))+
  facet_wrap(vars(scene),dir = "v",labeller = labeller(scene = scene_labs))+ # temporary for report
...
tt_list <- c("tall_trees","popufre","saligoo")
veg_names_full <-ordered(c("Prosopsis velutina","Drought Tolerant Shrubs","Tall Trees","Salix gooddingii","Populus fremontii"))
veg_labs <- c("prosvel"="Prosopsis velutina","drgt_tol_shrubs"="Drought Tolerant Shrubs","tall_trees"="Tall Trees","saligoo"="Salix gooddingii","popufre"="Populus fremontii")
# add criteria to % change table
veg_summary_tab <- inner_join(veg_pc_tab,veg_m_tab,by=c("group","scene")) %>%
  mutate(plt_order = ifelse(group %in% tt_list,1,2))
```
...
f_plt <- ggplot(data = fish_chg_mean, aes(x = group, y = per_chg, 
  ...
  #facet_wrap(vars(scene),dir = "v",labeller = labeller(scene =scene_names_full))+
    facet_wrap(vars(scene),dir = "v",labeller = labeller(scene =scene_labs))+ # temp for report
  {if(length(lifestages)>1) scale_alpha_discrete(range = c(1,0.35))}+
  {if(length(lifestages)==1) scale_alpha_discrete(guide="none",range = c(1,1))}+
  xlab(element_blank())+
  ylab("% Change in Habitat Area")
...
v_plt <- ggplot(data = veg_summary_tab, aes(x = reorder(group,plt_order), y = per_chg, fill = criteria))+
  theme_few()+
  scale_fill_brewer(palette = "Set1")+
  geom_bar(stat = "identity")+
  coord_flip()+
  #scale_x_discrete(labels = unique(veg_summary_tab$group))+
  scale_x_discrete(labels = veg_labs)+
  theme(legend.position = "right",
        text=element_text(size=14,face = "bold",color = "black"),
        axis.text.x = element_text(colour = "black",face="plain"),
          axis.text.y = element_text(colour = "black",face="plain"))+
