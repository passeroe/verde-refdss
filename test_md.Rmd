...
veg_names_full <- c("Drought Tolerant Shrubs", "Tall Trees","Populus fremontii","Prosopsis velutina","Salix gooddingii")
...
ggplot(data = fish_pc_tab, aes(x = factor(month), y = per_chg, 
...
fish_chg_mean <- fish_chg_mean %>%
  mutate(plt_order = ifelse(group %in% native_list,1,2))  #non-natives plotted at the end 
fish_chg_mean$criteria <- as.factor(fish_chg_mean$criteria) 
...
#sec_labs <- c("Smallmouth-Redeye Bass","Red Shiner","Speckled Dace","Sonora Sucker","Roundtail Chub","Desert Sucker")
...
# veg labels for report only
#veg_labs_full <- c("Prosopsis velutina","Drought Tolerant Shrubs", "Tall Trees","Populus fremontii","Salix gooddingii")
