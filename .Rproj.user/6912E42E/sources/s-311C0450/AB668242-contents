...
iric.process.smr <- function(m,delim_list,reach_wd,dem,setRes,x_loc,y_loc) {
  ...
  smr_elev <- raster(paste(reach_wd,dem,sep = ""))
  ...
  smr_q_s <- stack(lapply(delim_list,function(i) rasterize(x = i[,c(x_loc,y_loc)],y = r, field = ifelse(i[,depth] == 0,NA,i[,m]),fun = mean)))
