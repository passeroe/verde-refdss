...
# Last edited by Elaina Passero 1/24/20
...
plot.10day.ts <- function(a,species,scene_fish_out,scene_list,NormalizeByL,post_tag){
  ...
  if(NormalizeByL == "Yes"){
    ...
    ts_min_plt <- ggplot(ten_day_min_pt,aes(x=fill_date,y=mon_avg_low_area,color=scene))+
      ...
      labs(y=bquote('Minimum 10-day Habitat Area (Normalized)'),x="Year",title=paste(reach_name,", ",species,sep=""),color="")+
      scale_x_date(date_labels = "%Y")
    ts_min_plt
    ggsave(paste(post_tag,"_",species,"_","10day_hab_ts.png",sep=""),width=7, height=5,units = "in")
  } else{
    # plot time series of 10 day min area
    ts_min_plt <- ggplot(ten_day_min_pt,aes(x=fill_date,y=mon_avg_low_area,color=scene))+
      geom_line()+
      ...
      theme(text=element_text(size=12,face = "bold"),
            ...
            axis.text.x = element_text(colour = "black",face="plain",angle = 45,hjust=1),
            axis.text.y = element_text(colour = "black",face="plain"))+
      labs(y=bquote('Minimum 10-day Habitat Area'),x="Year",title=paste(reach_name,", ",species,sep=""),color="")+
      scale_x_date(date_labels = "%Y")
    ts_min_plt
    ggsave(paste(reach_wd,"dss_outputs/figures/",post_tag,"_",species,"_","10day_hab_ts.png",sep=""),width=7, height=5,units = "in")
  }
  return(ts_min_plt)
}
